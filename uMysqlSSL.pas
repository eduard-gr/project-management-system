...
uses sysutils{, Dialogs}; //for debug
...
begin
     ...
     begin
          //showmessage(format('verify error:num=%d : %s',[err,X509_verify_cert_error_string(err)]));
          if (depth<=0) then
             ok:=0
          else
             ok:=1;
...
begin
     ...
     if (dh= nil) then
        exit;
     dhdummy(dh^).p:=BN_bin2bn(@dh512_p,sizeof(dh512_p),nil); //set p
     dhdummy(dh^).g:=BN_bin2bn(@dh512_g,sizeof(dh512_g),nil); //set g
     if ((dhdummy(dh^).p = nil) or (dhdummy(dh^).g = nil)) then //any errors?
     begin
          dh_free(dh);
          exit;
     end;

...
uses sysutils, uMysqlErrors, uMysqlCT, uMysqlVio;
...
type
    TMysqlNet = class (TObject)
                    ...
                    function net_open( VioType:TEnumVioType; host:string; unix_socket:string; port:longint; connect_timeout:cardinal; trysock:boolean):longint;
                    ...
                    function net_safe_read:longint;
                    function net_vio_poll_read(timeout:cardinal):boolean;
                    {$IFDEF HAVE_SSL}
...
//zlib imports
{$IFDEF _WIN_}
{.$L c:\2a\zlib\compress.obj}
{.$L c:\2a\zlib\uncompr.obj}
{.$L c:\2a\zlib\deflate.obj}
{.$L c:\2a\zlib\adler32.obj}
{.$L c:\2a\zlib\trees.obj}

{.$L c:\2a\zlib\inflate.obj}
{.$L c:\2a\zlib\infblock.obj}
{.$L c:\2a\zlib\infcodes.obj}
{.$L c:\2a\zlib\inffast.obj}
{.$L c:\2a\zlib\inftrees.obj}
{.$L c:\2a\zlib\infutil.obj}
{.$L c:\2a\zlib.lib}
{.&OrgName+}
function compress(dest:pointer; destLen: pointer; const source:pointer; sourceLen:longint):longint;stdcall;external 'zlib.dll' name 'compress';
function uncompress(dest:pointer; destlen:pointer; source:pointer;sourceLne:longint):longint;stdcall;external 'zlib.dll' name 'uncompress';
{$ELSE}
function compress(dest:pointer; destLen: pointer; const source:pointer; sourceLen:longint):longint;
cdecl; external 'libz.so' name 'compress';
function uncompress(dest:pointer; destlen:pointer; source:pointer;sourceLne:longint):longint;
cdecl; external 'libz.so' name 'uncompress';
{$ENDIF}
     ...
     begin
          ...
          begin
               ...
               if len1<MYSQL_ERRMSG_SIZE-1 then //save the error
                  fvio.last_error:=copy(strpas(p),1,len1)
               else
                   fvio.last_error:=copy(strpas(p),1,MYSQL_ERRMSG_SIZE-1);
          end
          else
              begin
                   fvio.last_errno:=CR_UNKNOWN_ERROR;
...
////////////////////////////////////////////////////////////////////////////////
// wrapper for vio poll read
function TMysqlNet.net_vio_poll_read(timeout: cardinal): boolean;
begin
     result:=fvio.vio_poll_read(timeout);
end;
